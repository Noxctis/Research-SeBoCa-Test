...
for step in steps:
    ...
    # Allow 4 seconds for transient response to settle before calculating steady-state metrics
    ...
    if len(dct_stable) > 0 and len(cet_stable) > 0:
        ...
        # Calculate errors
        dct_err = dct_stable[target_col] - dct_stable[rpm_dct]
        cet_err = cet_stable[target_col] - cet_stable[rpm_cet]
        
        stats.append({
            'Target RPM': target_val,
            'DCT RMSE': np.sqrt((dct_err**2).mean()),
            'CET RMSE': np.sqrt((cet_err**2).mean()),
            'DCT Error SD': dct_err.std(),
            'CET Error SD': cet_err.std(),
            'DCT SS Error %': (np.abs(dct_err.mean()) / target_val) * 100,
            'CET SS Error %': (np.abs(cet_err.mean()) / target_val) * 100
        })
...
plt.plot(df_dct[t_dct], df_dct[rpm_dct], color='#d95f02', alpha=0.7, label='DCT Velocity')
plt.plot(df_cet[t_cet], df_cet[rpm_cet], color='#1b9e77', alpha=0.9, label='CET Velocity')
plt.plot(df_cet[t_cet], df_cet[target_col], color='black', linestyle=':', linewidth=3.0, label='Setpoint')
...
plt.plot(df_dct_zoom[t_dct], df_dct_zoom[rpm_dct], color='#d95f02', alpha=0.9, drawstyle='steps-post', label='DCT Response')
plt.plot(df_cet_zoom[t_cet], df_cet_zoom[rpm_cet], color='#1b9e77', drawstyle='steps-post', label='CET Response')
plt.axhline(target_val, color='black', linestyle=':', linewidth=3.0, label=f'Target ({target_val:.0f} RPM)')

plt.title(r'Steady-State PI Actuation ($T_s \approx 10$ ms)', fontweight='bold')
plt.xlabel('Time (s)')
plt.ylabel('Velocity (RPM)')
...
y_range = y_max - y_min
if y_range > 0:
    plt.ylim(y_min - 0.2 * y_range, y_max + 0.3 * y_range) # Extra top padding for text box

# Extract stats for the zoomed step to annotate the plot
step_stats = stats_df[stats_df['Target RPM'] == target_val].iloc[0]
stat_text = (
    f"DCT Metrics:\n"
    f"  RMSE: {step_stats['DCT RMSE']:.2f} RPM\n"
    f"  " + r"$\sigma$" + f": {step_stats['DCT Error SD']:.2f} RPM\n\n"
    f"CET Metrics:\n"
    f"  RMSE: {step_stats['CET RMSE']:.2f} RPM\n"
    f"  " + r"$\sigma$" + f": {step_stats['CET Error SD']:.2f} RPM"
)

# Place text box in the top right
plt.text(0.97, 0.95, stat_text, transform=plt.gca().transAxes, fontsize=12,
...
# ==========================================
# FIGURE 3: Error RMSE Bar Chart
# ==========================================
# Switched from StdDev to RMSE for standard control systems evaluation
fig3 = plt.figure(figsize=(12, 6.75))
bar_width = 0.35
index = np.arange(len(stats_df))

plt.bar(index, stats_df['DCT RMSE'], bar_width, label='DCT RMSE', color='#d95f02', edgecolor='black', linewidth=1.5)
plt.bar(index + bar_width, stats_df['CET RMSE'], bar_width, label='CET RMSE', color='#1b9e77', edgecolor='black', linewidth=1.5)
plt.xlabel('Target Velocity Setpoint (RPM)')
plt.ylabel('Root Mean Square Error (RPM)')
plt.title('Control Loop Tracking Accuracy (RMSE)', fontweight='bold')
...
print("--- PI CONTROLLER STEADY-STATE TRACKING STATISTICS ---")
